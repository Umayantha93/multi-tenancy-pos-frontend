...
import { Boxes, ChartNoAxesCombined, ClipboardList, Gauge, LogOut, Menu, ReceiptText, ShieldCheck, Users, Wrench, X } from "lucide-react";
import { api, clearSession, currentFeatures, currentUser, User } from "@/lib/api";
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[240px_1fr]">
      <aside className={`no-print fixed inset-y-0 left-0 z-40 flex w-[240px] flex-col bg-[#242723] text-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-20 items-center justify-between border-b border-white/10 px-5"><Link href="/dashboard" className="flex min-w-0 items-center gap-3"><span className="grid size-10 shrink-0 place-items-center bg-[#f5c842] text-[#20221f]"><Wrench size={20} /></span><div className="min-w-0"><strong className="block truncate font-display text-xl uppercase">{user?.tenant?.business_name ?? "Workshop"}</strong><p className="text-[10px] uppercase text-white/40">{user?.tenant?.business_type ?? "Business"} operations</p></div></Link><button onClick={() => setOpen(false)} className="lg:hidden" aria-label="Close navigation"><X /></button></div>

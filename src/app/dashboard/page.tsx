...
export default function DashboardPage() {
  ...
  return <AppShell title="Workshop overview" eyebrow={new Intl.DateTimeFormat("en-LK", { dateStyle: "full" }).format(new Date())} action={can("admit_vehicle") ? <Link href="/vehicles/admit" className="flex h-10 items-center gap-2 bg-[#f5c842] px-3 text-sm font-semibold sm:px-4"><ClipboardPlus size={18} /><span className="hidden sm:inline">Admit vehicle</span></Link> : undefined}>
    {error && <ErrorMessage message={error} />}
    {!data && !error ? <PageState message="Reading the workshop floor..." /> : data && <>
